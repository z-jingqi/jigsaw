...
import { ChevronDown, ChevronRight, Edit3, Eye, FolderPlus, Gamepad2, GripVertical, Hexagon, ImageIcon, ImageUp, Pencil, Plus, Save, Trash2, Wand2 } from "lucide-react";
import { toast } from "sonner";
import { loadCatalog, loadLevel, saveCatalog, saveLevel, sourceUrl, uploadSource } from "./api";
...
import { generatePieces, mergePieces, sequentialId, slug, withNeighbors, zhI18n } from "./geometry";
...
function App() {
  ...
  function recordRename(operation: CatalogRenameOperation) {
    setRenames((current) => mergeRenameOperation(current, operation));
  }
...
function TreePanel(props: {
  ...
  onRename: (operation: CatalogRenameOperation) => void;
  onRenameSelection: (value: SelectedLevel) => void;
}) {
  const { catalog, statuses, expanded, selected, editMode, onExpanded, onSelect, onClearSelection, onChange, onEditMode, onRename, onRenameSelection } = props;
  ...
  function addTopic() {
    ...
    onChange({ ...catalog, topics: [...catalog.topics, { id, name, name_i18n: zhI18n(name), cover: "", sort_order: catalog.topics.length, groups: [] }] });
  ...
  function remapExpandedTopic(fromTopicId: string, toTopicId: string) {
    const next: Record<string, boolean> = {};
    for (const [key, value] of Object.entries(expanded)) {
      next[key.replace(`topic:${fromTopicId}`, `topic:${toTopicId}`).replace(`group:${fromTopicId}:`, `group:${toTopicId}:`)] = value;
    }
    onExpanded(next);
  }

  function remapExpandedGroup(topicId: string, fromGroupId: string, toGroupId: string) {
    const next: Record<string, boolean> = {};
    for (const [key, value] of Object.entries(expanded)) {
      next[key.replace(`group:${topicId}:${fromGroupId}`, `group:${topicId}:${toGroupId}`)] = value;
    }
    onExpanded(next);
  }

  function updateTopicId(topic: CatalogTopic, value: string) {
    const id = slug(value, topic.id);
    if (id === topic.id) return;
    if (catalog.topics.some((item) => item.id === id)) {
      toast.error(`主题 ID 已存在：${id}`);
      return;
    }
    onChange({
      ...catalog,
    ...
    remapExpandedTopic(topic.id, id);
    onRename({ kind: "topic", fromTopicId: topic.id, toTopicId: id });
    if (selected?.topicId === topic.id) onRenameSelection({ ...selected, topicId: id });
  }

  function updateGroupId(topic: CatalogTopic, group: CatalogGroup, value: string) {
    const id = slug(value, group.id);
    if (id === group.id) return;
    if (topic.groups.some((item) => item.id === id)) {
      toast.error(`分组 ID 已存在：${id}`);
      return;
    }
    onChange({
      ...catalog,
              ...
              groups: item.groups.map((candidate) =>
                candidate.id === group.id
                  ? {
                      ...candidate,
                      id,
                      levels: candidate.levels.map((level) => withLevelPaths(topic.id, id, level)),
                    }
                  : candidate,
              ),
            }
    ...
    remapExpandedGroup(topic.id, group.id, id);
    onRename({ kind: "group", topicId: topic.id, fromGroupId: group.id, toGroupId: id });
    if (selected?.topicId === topic.id && selected.groupId === group.id) onRenameSelection({ ...selected, groupId: id });
  }

  function updateLevelId(topic: CatalogTopic, group: CatalogGroup, level: CatalogLevel, value: string) {
    const id = slug(value, level.id);
    if (id === level.id) return;
    if (group.levels.some((item) => item.id === id)) {
      toast.error(`关卡 ID 已存在：${id}`);
      return;
    }
    onChange({
      ...catalog,
                ...
                candidate.id === group.id
                  ? {
                      ...
                      levels: candidate.levels.map((candidateLevel) =>
                        candidateLevel.id === level.id ? withLevelPaths(topic.id, group.id, { ...candidateLevel, id }) : candidateLevel,
                      ),
                    }
                  : candidate,
              ),
    ...
    onRename({ kind: "level", topicId: topic.id, groupId: group.id, fromLevelId: level.id, toLevelId: id });
    if (selected?.topicId === topic.id && selected.groupId === group.id && selected.levelId === level.id) onRenameSelection({ ...selected, levelId: id });
  }
  ...
  function addGroup(topic: CatalogTopic) {
    ...
    onChange({
      ...catalog,
      topics: catalog.topics.map((item) =>
        item.id === topic.id ? { ...item, groups: [...item.groups, { id, name, name_i18n: zhI18n(name), sort_order: item.groups.length, levels: [] }] } : item,
      ),
    });
  ...
  function deleteSelected(target: DeleteTarget) {
    if (target.kind === "topic") {
      onChange({ ...catalog, topics: ordered(catalog.topics.filter((item) => item.id !== target.topic.id)) });
      if (selected?.topicId === target.topic.id) onClearSelection();
      return;
    }
    if (target.kind === "group") {
      onChange({
        ...catalog,
        topics: catalog.topics.map((item) => (item.id === target.topic.id ? { ...item, groups: ordered(item.groups.filter((candidate) => candidate.id !== target.group.id)) } : item)),
      });
      if (selected?.topicId === target.topic.id && selected.groupId === target.group.id) onClearSelection();
      return;
    }
    onChange({
      ...catalog,
      topics: catalog.topics.map((item) =>
        item.id === target.topic.id
          ? {
              ...item,
              groups: item.groups.map((candidate) =>
                candidate.id === target.group.id ? { ...candidate, levels: ordered(candidate.levels.filter((levelItem) => levelItem.id !== target.level.id)) } : candidate,
              ),
            }
          : item,
      ),
    });
    if (selected?.topicId === target.topic.id && selected.groupId === target.group.id && selected.levelId === target.level.id) onClearSelection();
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (active[0] === "topic") {
      ...
      onChange({ ...catalog, topics: ordered(arrayMove(catalog.topics, oldIndex, newIndex)) });
      return;
    }
    ...
    if (active[0] === "group") {
      ...
      onChange({
        ...
        topics: catalog.topics.map((topic) => {
          ...
          return { ...topic, groups: ordered(arrayMove(topic.groups, oldIndex, newIndex)) };
        }),
      });
      return;
    }
          ...
          topic.id === topicId
            ? {
                ...
                groups: topic.groups.map((group) => {
                  ...
                  return { ...group, levels: ordered(arrayMove(group.levels, oldIndex, newIndex)) };
                }),
              }
            : topic,
        ),
                  ...
                  <SortableTreeRow id={`topic:${topic.id}`} editMode={editMode}>
                    {({ handleProps }) => (
                      <TreeRow
                        ...
                        onId={(id) => updateTopicId(topic, id)}
                            ...
                            <SortableTreeRow id={`group:${topic.id}:${group.id}`} editMode={editMode}>
                              {({ handleProps }) => (
                                <TreeRow
                                  ...
                                  onId={(id) => updateGroupId(topic, group, id)}
                                    ...
                                    <SortableLevelRow key={level.id} id={`level:${topic.id}:${group.id}:${level.id}`} editMode={editMode}>
                                      {({ handleProps }) => (
                                        <LevelTreeRow
                                          ...
                                          onId={(id) => updateLevelId(topic, group, level, id)}
...
function TreeRow(props: {
  ...
  return (
    <div className={cn("flex min-h-10 items-center gap-1 rounded-md px-1", !props.isDraggingTree && "hover:bg-accent")} style={{ paddingLeft: `${props.depth * 18 + 4}px` }}>
      {props.editMode && (
        <button className="grid h-7 w-6 cursor-grab place-items-center text-muted-foreground active:cursor-grabbing" {...props.handleProps} title="拖拽排序">
      ...
      {props.editMode ? (
        <>
          <Input className="h-8 w-28 shrink-0 font-mono text-[11px]" value={props.idValue} onChange={(event) => props.onId(event.target.value)} />
          <Input className={cn("h-8 min-w-0 flex-1 border-transparent bg-transparent px-2 shadow-none focus-visible:bg-background", props.strong && "font-medium")} value={props.name} onChange={(event) => props.onName(event.target.value)} />
        </>
      ) : (
      ...
      {props.editMode && (
        <>
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={props.onAdd} title={props.addTitle}>
            <Plus size={14} />
          </Button>
...
function LevelTreeRow(props: {
  ...
  return (
    <div className={cn("ml-8 flex h-9 items-center gap-2 rounded-md px-2 text-muted-foreground", !props.isDraggingTree && "hover:bg-accent hover:text-foreground", props.active && "bg-accent text-foreground")}>
      ...
      {props.editMode && <Input className="h-7 w-24 shrink-0 font-mono text-[11px]" value={props.level.id} onChange={(event) => props.onId(event.target.value)} />}
      <button className="min-w-0 flex-1 truncate text-left" onClick={props.onSelect}>{props.level.title}</button>
      {props.status?.hasSource && <ImageIcon className="h-4 w-4 shrink-0 text-success" aria-label="已有图片" />}
...
function LevelDetails(props: {
  ...
  const { catalog, target, level, status, editMode, onLevel, onCatalog, onUpload, onSave, onEditPolygon } = props;
      ...
      <div className="grid grid-cols-[1fr_320px] gap-8">
        <div className="space-y-4">
          ...
          <div className="border-y border-border py-3">
            ...
            <StatusLine label="凹凸" value="Godot 自动生成 6 x 8" />
            <StatusLine label="Swap" value="Godot 自动生成 3 x 4" />
          </div>
...
function PolygonEditor(props: {
  ...
  const [targetCount, setTargetCount] = React.useState(Math.max(12, level.modes.polygon?.pieces?.length || 24));
