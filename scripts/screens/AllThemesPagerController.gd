class_name AllThemesPagerController
extends RefCounted
...
func set_page_width(page_width: float) -> void:
	_page_width = maxf(1.0, page_width)
...
func drag_by(delta_x: float, elapsed_override := -1.0) -> void:
	...
	_emit_visual(direction, clampf(absf(_drag_total) / _page_width, 0.0, 1.0), _drag_total)
...
func end() -> void:
	...
	var should_commit := (
		_can_move(direction) and (ratio >= COMMIT_RATIO or absf(_velocity) >= FLING_VELOCITY)
	)
