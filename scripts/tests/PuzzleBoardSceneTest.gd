...
func _run() -> void:
	...
	_check(
		(
			board.get_node_or_null("VisualHost") != null
			and board.get_node_or_null("InputHost") != null
		),
		"static_visual_input_hosts_declared"
	)
	...
	await process_frame
	_check(
		board.get_node_or_null("WorldRoot") == world and board.get_node_or_null("TrayRoot") == tray,
		"clear_preserves_static_hosts"
	)
	_check(
		world.get_child_count() == 0 and tray.get_child_count() == 0,
		"clear_releases_dynamic_host_children"
	)
	board.queue_free()
	await process_frame
