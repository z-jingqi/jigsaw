...
func _run() -> void:
	...
	_check(
		(
			screen.get_node("SafeArea/Content/Header/ThemeTitle").text
			== "The Classic of Mountains and Seas"
		),
		"header_title"
	)
	_check(
		screen.get_node("SafeArea/Content/Scroll/GridContent").get_child_count() < 36,
		"virtualized_cards"
	)
	...
	_check(
		(
			interrupted_card.get_node_or_null("unlock_outline") == null
			and interrupted_card.scale.is_equal_approx(Vector2.ONE)
		),
		"offscreen_unlock_releases"
	)
	screen.get_node("SafeArea/Content/Scroll").scroll_vertical = 0
	screen.refresh_view_model(_view_model(2, true))
	await process_frame
	_check(
		screen.get_node("SafeArea/Content/Scroll/GridContent").get_child_count() <= 2,
		"refresh_without_stack_rebuild"
	)
	_selected = ""
	...
	_check(
		(
			unlocked_card.get_node_or_null("unlock_outline") == null
			and unlocked_card.scale.is_equal_approx(Vector2.ONE)
		),
		"unlock_sequence_released"
	)
	...
	_check(
		(
			reduced_card.get_node_or_null("unlock_outline") == null
			and screen.active_motion_count() == 0
		),
		"reduced_motion_unlock_finishes_immediately"
	)
	...
	_check(
		(
			screen.get_node("SafeArea/Content/Header").modulate.a == 1.0
			and screen.debug_active_card_count() > 0
		),
		"reentry_restores_visible_screen"
	)
...
func _view_model(
	count: int, first_locked := false, first_new := false, newly_unlocked_index := 0
) -> Variant:
	...
	for index in count:
		cards.append(
			ViewModels.LevelCardViewModel.new(
				{
					"level_id": "level_%02d" % index,
					"title": "Level %02d" % index,
					"thumbnail": texture,
					"locked": first_locked and index == 0,
					"recommended": index == 0,
					"newly_unlocked": first_new and index == newly_unlocked_index,
					"modes":
					[
						ViewModels.ModeStatusViewModel.new(
							&"polygon", "Polygon", &"completed", &"replay", true
						),
						ViewModels.ModeStatusViewModel.new(
							&"knob", "Knob", &"not_started", &"start", true
						)
					]
				}
			)
		)
	return ViewModels.LevelListViewModel.new(
		{
			"revision": 1,
			"theme_id": "topic_01",
			"theme_title": "The Classic of Mountains and Seas",
			"theme_progress": progress,
			"focus_level_id": "level_00",
			"levels": cards
		}
	)
