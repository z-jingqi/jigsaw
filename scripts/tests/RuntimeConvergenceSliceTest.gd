...
func _run() -> void:
	...
	await create_timer(0.40).timeout
	_check(
		str(game.debug_state_snapshot().get("modal", "")) == "settings",
		"settings_is_navigator_modal"
	)
	...
	state = game.debug_state_snapshot()
	_check(
		str(state.get("screen", "")) == "levels" and str(state.get("modal", "")).is_empty(),
		"modal_restores_levels"
	)
	var enter := game.debug_execute(
		"enter_level", {"topic_id": "topic_01", "level_id": "shanhai_01", "mode": "polygon"}
	)
	...
	_check(
		(
			str(state.get("topic_id", "")) == "topic_01"
			and str(state.get("level_id", "")) == "shanhai_01"
		),
		"gameplay_state_is_deterministic"
	)
	...
	await create_timer(0.40).timeout
	_check(
		str(game.debug_state_snapshot().get("modal", "")) == "completion",
		"completion_is_navigator_modal"
	)
	...
	_check(
		str(game.debug_state_snapshot().get("screen", "")) == "levels",
		"completion_returns_to_levels"
	)
