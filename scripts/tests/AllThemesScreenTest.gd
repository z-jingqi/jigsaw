...
func _run() -> void:
	...
	_check(
		(
			...
			and animation_player.has_animation(&"enter")
			and animation_player.has_animation(&"exit")
		),
		"screen_timelines"
	)
	...
	_check(
		(
			not incomplete.get_node("Margin/Content/Progress/Journey").visible
			and not incomplete.get_node("Margin/Content/Progress/NumericCompletion").visible
		),
		"incomplete_numeric_only"
	)
	_check(
		(
			complete.get_node("Margin/Content/Progress/Numeric").text == "5 / 5"
			and complete.get_node("Margin/Content/Progress/NumericCompletion").visible
		),
		"complete_numeric_mark"
	)
	_check(
		(
			empty.get_node("Margin/Content/Progress/Numeric").text == "0 / 0"
			and not empty.get_node("Margin/Content/Progress/NumericCompletion").visible
		),
		"zero_total_not_complete"
	)
	...
	_check(
		(
			not screen.get_node("SafeArea/Content/Header/CloseButton").disabled
			and screen.get_node("SafeArea/Content/Header").modulate.a == 1.0
		),
		"reactivation_unlocks_after_selection"
	)
	...
	_check(
		_selected_theme == "topic_02" and screen.active_motion_count() == 0,
	...
	var progress := ViewModels.ThemeProgressViewModel.new(
		{
			...
			"total_modes": total,
			"ratio": ratio,
			"paw_count": 5 if ratio >= 0.8 else 1,
			"is_complete": total > 0 and completed == total
		}
	)
	return ViewModels.ThemeCardViewModel.new(
		{
			"theme_id": theme_id,
			"title": title,
