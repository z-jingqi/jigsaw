...
func _run() -> void:
	...
	levels = navigator.push(&"levels", {"theme_id": "topic_01"})
	_check(bool(levels.get("ok", false)), "open_levels_reduced_motion")
	...
	_check(bool(pop_result.get("ok", false)), "close_levels_reduced_motion")
	transition_host.finish_active_to_target()
	await process_frame
	navigator.set_reduced_motion(false)
	for index in 5:
		levels = navigator.push(&"levels", {"theme_id": "topic_01"})
		_check(bool(levels.get("ok", false)), "rapid_open_%d" % index)
		navigator.cancel_active_transition()
		await process_frame
	_check(
		(
		...
		"rapid_navigation_cycles_release_views"
	)
