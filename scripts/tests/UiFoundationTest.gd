...
func _run() -> void:
	...
	_check(
		ThemeResource.get_stylebox(&"focus", &"Button").border_width_left == 3, "theme_focus_ring"
	)
	_check(
		ThemeResource.get_stylebox(&"focus", &"CheckButton").border_width_left == 3,
		"toggle_focus_ring"
	)
	_check(TokenResource.minimum_touch_size >= 44.0, "touch_token")
	_check(
		(
			TokenResource.theme_for_variant(TokenResource.TextVariant.ON_LIGHT) != null
			and TokenResource.theme_for_variant(TokenResource.TextVariant.ON_DARK) != null
		),
		"theme_variants"
	)
	...
	_check(
		(
			is_equal_approx(MotionResource.progress_cat_duration, 0.26)
			and is_equal_approx(MotionResource.numeric_completion_duration, 0.22)
		),
		"progress_motion_tokens"
	)
	...
	progress.set_progress_data(_progress(1, 5, 0.2, 1, false))
	_check(
		progress.get_node("Numeric").visible and not progress.get_node("Journey").visible,
		"numeric_hides_journey"
	)
...
func _progress(
	...
	return {
		...
		"accessibility_text": "%d / %d" % [completed, total]
	}
...
func _test_component_scenes() -> void:
	...
	await process_frame
	_check(
		icon_button.custom_minimum_size.x >= 44.0 and icon_button.custom_minimum_size.y >= 44.0,
		"icon_touch_target"
	)
	...
	_check(
		is_equal_approx(icon_button.scale.x, MotionResource.icon_press_scale), "icon_press_feedback"
	)
	...
	level_card.set_view_model(
		{
			...
			"locked": false,
			"modes": [{"status": "complete"}, {"status": "available"}]
		}
	)
	_check(
		(
			level_card.level_id == "shanhai_08"
			and level_card.get_node("Margin/Content/Status").text == "complete · available"
		),
		"level_card_view_model"
	)
	settings_row.configure(&"music", "Music", true)
	_check(
		settings_row.setting_key == &"music" and settings_row.get_node("Toggle").button_pressed,
		"settings_row_configuration"
	)
	_check(safe_area.get_theme_constant("margin_left") >= 20, "safe_area_compact_margin")
	root.size = Vector2i(768, 1024)
	await process_frame
	var regular_content_width: int = (
		root.size.x
		- safe_area.get_theme_constant("margin_left")
		- safe_area.get_theme_constant("margin_right")
	)
	_check(
		safe_area.get_theme_constant("margin_left") >= 32 and regular_content_width <= 704,
		"safe_area_regular_max_width"
	)
	FocusNavigationScript.configure_linear([icon_button, pill_button])
	_check(
		icon_button.focus_next == icon_button.get_path_to(pill_button), "focus_linear_navigation"
	)
	_check(
		(
			modal_shell.get_node("AnimationPlayer").has_animation(&"RESET")
			and modal_shell.get_node("AnimationPlayer").has_animation(&"open")
			and modal_shell.get_node("AnimationPlayer").has_animation(&"close")
		),
...
func _test_modal_interruption() -> void:
	...
	for iteration in 5:
		...
		await process_frame
	_check(
		root.find_children("RapidModal*", "Control", true, false).is_empty(),
		"modal_rapid_cycles_release"
	)
