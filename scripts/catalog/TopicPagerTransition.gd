...
func slide(
	from_x: float, target_x: float, duration: float, setter: Callable, finished: Callable
) -> void:
	cancel()
...
func fade_out(page: Control, direction: int, page_width: float, finished: Callable) -> void:
	...
	tween.tween_property(page, "modulate:a", 0.0, 0.17).set_trans(Tween.TRANS_CUBIC).set_ease(
		Tween.EASE_IN
	)
	(
		tween
		. tween_property(
			page, "position:x", start_position.x - float(direction) * page_width * 0.08, 0.18
		)
...
func fade_in(page: Control, direction: int, page_width: float, finished: Callable) -> void:
	...
	page.modulate.a = 0.0
	tween = game.create_tween().set_parallel(true)
	tween.tween_property(page, "modulate:a", 1.0, 0.22).set_trans(Tween.TRANS_CUBIC).set_ease(
		Tween.EASE_OUT
	)
	(
		tween
		. tween_property(page, "position:x", final_position.x, 0.24)
		. set_trans(Tween.TRANS_CUBIC)
