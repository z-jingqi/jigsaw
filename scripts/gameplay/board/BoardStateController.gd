...
const BoardSessionIdentityScript := preload(
	"res://scripts/gameplay/runtime/BoardSessionIdentity.gd"
)
...
func snapshot() -> Dictionary:
	...
	var snapshot := {
		...
		"tray":
		{
			...
			(
				0.0
				if tray_max_scroll <= 0.0
				else clampf(board.tray_scroll_offset / tray_max_scroll, 0.0, 1.0)
			),
		},
	}
...
func session_snapshot(theme_id: String, level_id: String) -> Dictionary:
	...
	if board.current_mode == "swap":
		...
		ordered.sort_custom(
			func(left, right) -> bool: return int(left["slot_index"]) < int(right["slot_index"])
		)
		for tile in ordered:
...
func should_persist() -> bool:
	if board.completion_emitted:
		return false
	return (
		not board.swap_tiles.is_empty()
		if board.current_mode == "swap"
		else not board.groups.is_empty()
	)
...
func restore_group_state(snapshot: Dictionary) -> void:
	...
	for id_value in tray_order:
		...
		if (
			group != null
			and board.groups.has(group)
			and not group.locked
			and not board.tray_groups.has(group)
		):
	...
	board.tray_scroll_offset = (
		...
		else maxf(0.0, float(tray_state.get("scroll", 0.0)))
	)
...
func restore_swap_state(snapshot: Dictionary) -> void:
	...
	for item in tile_states:
		...
		if is_instance_valid(node):
			node.position = board._swap_slot_position(
				int(tile["slot_index"]), board._swap_cols(), board._swap_rows()
			)
	var ordered := tile_states.duplicate()
	ordered.sort_custom(func(a, b) -> bool: return int(a.get("z", 0)) < int(b.get("z", 0)))
	var next_tiles := []
	for item in ordered:
		var tile = by_correct.get(int(item.get("correct_index", -1)), null)
...
func notify_changed(immediate := false) -> void:
	...
	board.get_tree().create_timer(0.25).timeout.connect(
		func() -> void:
