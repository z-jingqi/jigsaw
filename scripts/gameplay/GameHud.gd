...
const SwapActionBarScript := preload("res://scripts/gameplay/SwapActionBar.gd")
const HINT_ICON_PATH := "res://assets/icons/lightbulb.svg"
...
func _init(owner: Node) -> void:
	game = owner
	swap_action_bar = SwapActionBarScript.new(owner)
...
func build(level_title: String) -> void:
	...
	var hint_button: Button = game._icon_button(
		game.repository.cached_texture(HINT_ICON_PATH),
	...
	var decoration_width := minf(
		...
		minf(
			...
			hint_button.position.x - (viewport_width * 0.5 + title_text_width * 0.5 + decoration_gap) - decoration_clearance,
		),
	)
	...
	var bottom_bar: Control = swap_action_bar.build(ui_scale)
...
func bottom_bar_height() -> float:
	return swap_action_bar.height() if game.current_mode == "swap" else 0.0
