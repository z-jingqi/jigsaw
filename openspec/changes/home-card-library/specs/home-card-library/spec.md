...
首页 SHALL 只显示单行主题名称，并在名称两侧以固定显示尺寸复用同一祥云素材；右侧 SHALL 水平镜像且不旋转。祥云容器 SHALL 使用纹理实际宽高比。标题 SHALL 按真实字体宽度缩小字号且不省略，窗口变化与主题切换时重新排版。
#### Scenario: Long English theme name
- **WHEN** 主题名称为 `the classic mountains and seas` 或同等长度文本
- **THEN** 完整名称保持单行且不与祥云重叠，左右祥云到实际文字边界的间距相同且固定，文字、间距与固定尺寸祥云组成的整体水平居中；短名称时祥云靠近，长名称时随文字边界向外。
